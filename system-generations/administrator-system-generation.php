...
# -------------------------------------------------------------
#
# Name       : administrator-system-generation
# Purpose    : This is to handle all of the administrator side of the system.
# Installer  : Default
#
# -------------------------------------------------------------
...
if(isset($_POST['type']) && !empty($_POST['type']) && isset($_POST['email_account']) && !empty($_POST['email_account'])){
    ...
    switch ($type) {
        ...
        case 'menu groups table':
            if (!$global_class->database_connection()) {
                echo 'Database Connection Error';
                exit;
            }

            $sql = $global_class->db_connection->prepare('SELECT external_id, menu_group_name, order_sequence FROM admin_menu_groups');
    
            if($sql->execute()){
                $menu_group_delete_access_right = $administrator_class->check_menu_access_rights($email_account, 2, 'delete');
                ...
                while($row = $sql->fetch()){
                    ...
                    $menu_group_id_encrypted = $global_class->encrypt_data($external_id);

                    if($menu_group_delete_access_right > 0){
                        $delete = '<button type="button" class="btn btn-icon btn-danger delete-menu-group" data-menu-group-id="' . $external_id . '" title="Delete Menu Group">
                    ...
                    $response[] = array(
                        'CHECK_BOX' => '<input class="form-check-input datatable-checkbox-children" data-delete="1" type="checkbox" value="'. $external_id .'">',
        ...
        case 'menu group menu item table':
            ...
            if(isset($_POST['menu_group_id']) && !empty($_POST['menu_group_id'])){
                ...
                $menu_group_write_access_right = $administrator_class->check_menu_access_rights($email_account, 2, 'write');
                $menu_item_write_access_right = $administrator_class->check_menu_access_rights($email_account, 3, 'write');
                $menu_item_delete_access_right = $administrator_class->check_menu_access_rights($email_account, 3, 'delete');
                $assign_menu_item_role_access = $administrator_class->check_system_action_access_rights($email_account, 1);

                $sql = $global_class->db_connection->prepare('SELECT external_id, menu_item_name, parent_id, order_sequence FROM admin_menu_item WHERE menu_group_id = :menu_group_id');
                ...
                if($sql->execute()){
                    while($row = $sql->fetch()){
                        ...
                        $parent_menu_item_details = $administrator_class->get_menu_item_details($parent_id);
                        $parent_menu_item_name = $parent_menu_item_details[0]['MENU_ITEM_NAME'] ?? null;

                        $external_id_encrypted = $global_class->encrypt_data($external_id); 

                        if($menu_item_write_access_right > 0 && $menu_group_write_access_right > 0){
                            $update = '<button type="button" class="btn btn-icon btn-info update-menu-item" data-external-id="'. $external_id .'" title="Edit Menu Item">
                        ...
                        if($menu_item_delete_access_right > 0 && $menu_group_write_access_right > 0){
                            $delete = '<button type="button" class="btn btn-icon btn-danger delete-menu-item" data-external-id="'. $external_id .'" title="Delete Menu Item">
                                                <i class="ti ti-trash"></i>
                                            </button>';
                        }
                        else{
                        ...
                        if($assign_menu_item_role_access > 0){
                            $assign = '<button type="button" class="btn btn-icon btn-warning assign-menu-item-role-access" data-external-id="'. $external_id .'" title="Assign Menu Item Role Access">
                        ...
                        $response[] = array(
                            'EXTERNAL_ID' => $external_id,
                            'MENU_ITEM_NAME' => '<a href="menu-item-form.php?id='. $external_id_encrypted .'">'. $menu_item_name . '</a>',
        ...
        case 'menu item table':
            ...
            if(isset($_POST['filter_menu_group_id']) && isset($_POST['filter_parent_id'])){
                ...
                $query = 'SELECT external_id, menu_item_name, menu_group_id, parent_id, order_sequence FROM admin_menu_item';
                ...
                $sql = $global_class->db_connection->prepare($query);
                ...
                if($sql->execute()){
                    while($row = $sql->fetch()){
                        ...
                        $menu_groups_details = $administrator_class->get_menu_groups_details($menu_group_id);
                        ...
                        $response[] = array(
                            ...
                            'ORDER_SEQUENCE' => $order_sequence,
                            'ACTION' => '<div class="d-flex gap-2">
                                                <a href="menu-item-form.php?id='. $external_id_encrypted .'" class="btn btn-icon btn-primary" title="View Menu Item">
                                                    <i class="ti ti-eye"></i>
                                                </a>
                                            '. $assign .'
                                            '. $delete .'
        ...
        case 'assign menu item role access table':
            ...
            if(isset($_POST['menu_item_id']) && !empty($_POST['menu_item_id'])){
                ...
                $sql = $global_class->db_connection->prepare('SELECT role_id, role_name FROM role');
                ...
                if($sql->execute()){
                    while($row = $sql->fetch()){
                        ...
                        $role_menu_access_rights = $administrator_class->get_role_menu_access_rights($menu_item_id, $role_id);
        ...
        case 'submenu item table':
            ...
            if(isset($_POST['external_id']) && !empty($_POST['external_id'])){
                $external_id = htmlspecialchars($_POST['external_id'], ENT_QUOTES, 'UTF-8');

                $sql = $global_class->db_connection->prepare('SELECT external_id, menu_item_name FROM admin_menu_item WHERE parent_id = :external_id');
                $sql->bindValue(':external_id', $external_id);
                ...
                if($sql->execute()){
                    while($row = $sql->fetch()){
                        ...
                        $external_id_encrypted = $administrator_class->encrypt_data($external_id);
        ...
        case 'file types table':
            ...
            $sql = $global_class->db_connection->prepare('SELECT file_type_id, file_type_name FROM file_types');
            ...
            if($sql->execute()){                    
                ...
                while($row = $sql->fetch()){
                    ...
                    $file_type_id_encrypted = $administrator_class->encrypt_data($file_type_id);
