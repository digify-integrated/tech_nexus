(function ($) {
  ...
  $(function () {
    ...
    const withTempDisable = (selector, ms = 800) => {
      const $btn = $(selector);
    ...
    // ---------------------------------------
    // 4) product_type handler (single source of truth)
    // ---------------------------------------
    $doc.on('change', '#product_type', function () {
      ...
      // Delivery price readonly rule
      const lockDelivery = ['Unit', 'Rental', 'Consignment', 'Fuel'].includes(productType);
      setReadonlyIfExists('#delivery_price', lockDelivery);

      // Insurance premium readonly rule
      const unlockInsurance = ['Refinancing', 'Restructure'].includes(productType);
      setReadonlyIfExists('#insurance_premium', !unlockInsurance);

      // Summary update (merged from your separate handler)
    ...
    // ---------------------------------------
    $doc.on('click', '#next-step', function () { withTempDisable(this); traverseTabs('next'); });
    $doc.on('click', '#previous-step', function () { withTempDisable(this); traverseTabs('previous'); });
    $doc.on('click', '#first-step', function () { withTempDisable(this); traverseTabs('first'); });
    $doc.on('click', '#last-step, #last-step2', function () { withTempDisable(this); traverseTabs('last'); });
...
const initValidatedForm = ({
  ...
  $form.validate({
    ...
    highlight,
    unhighlight,
    submitHandler: (form) => submit(form),
  });
};
  ...
  try {
    switch (transaction) {
      ...
      case 'get sales proposal renewal amount details': {
        ...
        const computeReadonlyRules = () => {
          // same rules you had, centralized:
        ...
        if (response.insuranceCoverageFourthYear > 0) {
          ...
          // Note: your original code targeted premium twice; keeping intent: coverage + premium.
          const r = computeReadonlyRules();
...
function traverseTabs(direction) {
  ...
  const validateAndSubmit = (formSelector) => {
    ...
    // If the form has no ENABLED inputs, skip validation/submission
    const hasEnabledInputs = $form.find(':input:not(:disabled)').length > 0;
    ...
    refinancing: {
      ...
      onEnter: () => {
        $('#sales-proposal-tab-4').text(
          product_type === 'Brand New' ? 'Brand New Details'
            : product_type === 'Restructure' ? 'Restructure Details'
              : 'Refinancing Details'
        );
      },
    },
  ...
  const visibleTabs = $tabs.not('.d-none').length || 1;
