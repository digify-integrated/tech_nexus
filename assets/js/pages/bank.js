    ...
    $(function() {
        ...
        $(document).on('click','.delete-bank',function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'POST',
                        url: 'controller/bank-controller.php',
                        dataType: 'json',
                        data: {
                            bank_id : bank_id, 
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                if (response.isInactive) {
                                    ...
                                    showNotification('Delete Bank Error', 'The bank does not exist.', 'danger');
                                    reloadDatatable('#bank-table');
                                }
                                else {
        ...
        $(document).on('click','#delete-bank',function() {
            ...
            if(bank_id.length > 0){
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.success) {
                                    showNotification('Delete Bank Success', 'The selected banks have been deleted successfully.', 'success');
                                        reloadDatatable('#bank-table');
                                }
                                else {
                                    if (response.isInactive) {
                ...
                showNotification('Deletion Multiple Bank Error', 'Please select the banks you wish to delete.', 'danger');
            }
        });

        $(document).on('click','#delete-bank-details',function() {
            const bank_id = $('#bank-id').text();
            const transaction = 'delete bank';
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                setNotification('Deleted Bank Success', 'The bank has been deleted successfully.', 'success');
        ...
        $(document).on('click','#duplicate-bank',function() {
            const bank_id = $('#bank-id').text();
            const transaction = 'duplicate bank';
    
            Swal.fire({
                title: 'Confirm Bank Duplication',
                text: 'Are you sure you want to duplicate this bank?',
                icon: 'info',
                showCancelButton: !0,
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.success) {
                                setNotification('Duplicate Bank Success', 'The bank has been duplicated successfully.', 'success');
                                window.location = 'bank.php?id=' + response.bankID;
                            }
                            else {
                                if (response.isInactive) {
                                    setNotification('User Inactive', response.message, 'danger');
                                    ...
                                    showNotification('Duplicate Bank Error', 'The bank does not exist.', 'danger');
                                    reloadDatatable('#bank-table');
                                }
                                else {
                                    showNotification('Duplicate Bank Error', response.message, 'danger');
                                }
                            }
                        },
                        error: function(xhr, status, error) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#bank_name_label').text(response.bankName);
                        $('#bank_identifier_code_label').text(response.bankIdentifierCode);
                    } 
                    else {
                        if(response.isInactive){
                            window.location = 'logout.php?logout';
