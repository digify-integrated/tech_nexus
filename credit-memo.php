...
function printCreditMemoCopy(TCPDF $pdf, string $summaryTable): void
{
    ...
    $pdf->MultiCell(0, 0, '<b><u>CREDIT MEMO</u></b>', 0, 'C', 0, 1, '', '', true, 0, true, true, 0);
    $pdf->Ln(8);
...
function generatePrint($sales_proposal_id)
{
    ...
    $sql = $databaseModel->getConnection()->prepare('
        ...
        WHERE sales_proposal_id = :sales_proposal_id
          AND reference_number NOT LIKE "%FOLLOW%" AND reference_number NOT LIKE "%tf%"
    ...
    foreach ($options as $row) {
        ...
        $list .= 'REF No.: ' . $reference_number . ' - PHP ' . number_format($deposit_amount, 2) . '<br/>';
    }
    ...
    $response = '
        <table border="1" width="100%" cellpadding="5" align="left">
            ...
            <tbody>
                <tr>
                    <td width="60%" style="text-align:left">TO APPLY DEPOSIT FROM CGMI TO ACCT:<br/> <br/>
                        ' . $list . '
                    </td>
