<?php
  ...
  $disbursementReadAccess = $userModel->checkMenuItemAccessRights($user_id, 134, 'read');
  $disbursementCreateAccess = $userModel->checkMenuItemAccessRights($user_id, 134, 'create');
  $disbursementWriteAccess = $userModel->checkMenuItemAccessRights($user_id, 134, 'write');
  $disbursementDeleteAccess = $userModel->checkMenuItemAccessRights($user_id, 134, 'delete');
  ...
  $disbursementCategory = 'disbursement voucher';

  if($disbursementCategory === 'disbursement check' || $disbursementCategory === 'disbursement voucher'){
    $payableClient = '';
      ...
      <div class="pc-content">
        ...
        <?php
          if($newRecord && $disbursementCreateAccess['total'] > 0){
            ...
            require_once('view/_journal_voucher.php');
          }
        ?>
      </div>
    </section>
