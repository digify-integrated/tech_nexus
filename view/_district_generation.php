<?php
require_once '../session.php';
require_once '../config/config.php';
require_once '../model/database-model.php';
require_once '../model/user-model.php';
require_once '../model/security-model.php';
require_once '../model/system-model.php';
require_once '../model/district-model.php';
require_once '../model/city-model.php';
require_once '../model/state-model.php';
require_once '../model/country-model.php';
require_once '../model/role-model.php';

$databaseModel = new DatabaseModel();
$systemModel = new SystemModel();
$userModel = new UserModel($databaseModel, $systemModel);
$districtModel = new DistrictModel($databaseModel);
$cityModel = new CityModel($databaseModel);
$stateModel = new StateModel($databaseModel);
$countryModel = new CountryModel($databaseModel);
$roleModel = new RoleModel($databaseModel);
$securityModel = new SecurityModel();

if(isset($_POST['type']) && !empty($_POST['type'])){
    $type = htmlspecialchars($_POST['type'], ENT_QUOTES, 'UTF-8');
    $response = [];
    
    switch ($type) {
        # -------------------------------------------------------------
        #
        # Type: district table
        # Description:
        # Generates the district table.
        #
        # Parameters: None
        #
        # Returns: Array
        #
        # -------------------------------------------------------------
        case 'district table':
            if(isset($_POST['filter_city']) && isset($_POST['filter_state']) && isset($_POST['filter_country'])){
                $filterCity = htmlspecialchars($_POST['filter_city'], ENT_QUOTES, 'UTF-8');
                $filterState = htmlspecialchars($_POST['filter_state'], ENT_QUOTES, 'UTF-8');
                $filterCountry = htmlspecialchars($_POST['filter_country'], ENT_QUOTES, 'UTF-8');

                $sql = $databaseModel->getConnection()->prepare('CALL generateDistrictTable(:filterCity, :filterState, :filterCountry)');
                $sql->bindValue(':filterCity', $filterCity, PDO::PARAM_INT);
                $sql->bindValue(':filterState', $filterState, PDO::PARAM_INT);
                $sql->bindValue(':filterCountry', $filterCountry, PDO::PARAM_INT);
                $sql->execute();
                $options = $sql->fetchAll(PDO::FETCH_ASSOC);
                $sql->closeCursor();

                $districtDeleteAccess = $userModel->checkMenuItemAccessRights($user_id, 17, 'delete');

                foreach ($options as $row) {
                    $districtID = $row['district_id'];
                    $districtName = $row['district_name'];
                    $cityID = $row['city_id'];
                    $stateID = $row['state_id'];
                    $countryID = $row['country_id'];

                    $cityName = $cityModel->getCity($cityID)['city_name'];
                    $stateName = $stateModel->getState($stateID)['state_name'];
                    $countryName = $countryModel->getCountry($countryID)['country_name'];

                    $districtIDEncrypted = $securityModel->encryptData($districtID);

                    $delete = '';
                    if($districtDeleteAccess['total'] > 0){
                        $delete = '<button type="button" class="btn btn-icon btn-danger delete-district" data-district-id="'. $districtID .'" title="Delete City">
                                            <i class="ti ti-trash"></i>
                                        </button>';
                    }

                    $response[] = [
                        'CHECK_BOX' => '<input class="form-check-input datatable-checkbox-children" type="checkbox" value="'. $districtID .'">',
                        'DISTRICT_NAME' => $districtName,
                        'CITY_ID' => $cityName,
                        'STATE_ID' => $stateName,
                        'COUNTRY_ID' => $countryName,
                        'ACTION' => '<div class="d-flex gap-2">
                                        <a href="district.php?id='. $districtIDEncrypted .'" class="btn btn-icon btn-primary" title="View Details">
                                            <i class="ti ti-eye"></i>
                                        </a>
                                        '. $delete .'
                                    </div>'
                    ];
                }

                echo json_encode($response);
            }
        break;
        # -------------------------------------------------------------
    }
}

?>