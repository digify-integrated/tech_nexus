...
if(isset($_POST['type']) && !empty($_POST['type'])){
    ...
    switch ($type) {
        ...
        case 'all sales proposal table2':
                ...
                echo json_encode($response);
        break;
        case 'all sales proposal table3':
                $filter_sale_proposal_status = $_POST['filter_sale_proposal_status'];
                $filter_product_type = $_POST['filter_product_type'];
                $filter_company = $_POST['filter_company'];
                $filter_user = $_POST['filter_user'];
                ...
                $viewOwnSalesProposal = $userModel->checkSystemActionAccessRights($user_id, 131);
                 ...
                 $sql = $databaseModel->getConnection()->prepare('CALL generateJobOrderSalesProposalTable(:filter_sale_proposal_status, :filter_product_type, :filter_company, :filter_user, :filter_created_date_start_date, :filter_created_date_end_date, :filter_released_date_start_date, :filter_released_date_end_date)');
                ...
                foreach ($options as $row) {
                    ...
                    $response[] = [
                        ...
                        'STATUS' => $salesProposalStatus,
                        'ACTION' => '<div class="d-flex gap-2">
                                        <a href="job-order.php?customer='. $securityModel->encryptData($customerID) .'&id='. $salesProposalIDEncrypted .'" class="btn btn-icon btn-primary" target="_blank" title="View Details">
                                            <i class="ti ti-eye"></i>
                                        </a>
                                    </div>'
                        ];
        ...
        case 'sales proposal for ci verification list':
            ...
            echo json_encode(['LIST' => $list]);
        break;
        case 'sales proposal for verification list':
            $sql = $databaseModel->getConnection()->prepare('SELECT * FROM sales_proposal WHERE sales_proposal_status IN ("For CI", "For Final Approval") AND for_ci_date IS NOT NULL AND ci_completion_date IS NOT NULL AND ci_recommendation_date IS NOT NULL AND ci_recommendation IS NOT NULL ORDER BY for_ci_date DESC');
            $sql->execute();
            $options = $sql->fetchAll(PDO::FETCH_ASSOC);
            $sql->closeCursor();
            ...
            foreach ($options as $row) {
                ...
                $productType = $row['product_type'];
                $productID = $row['product_id'];
                $ci_recommendation = $row['ci_recommendation'];
                $createdDate = $systemModel->checkDate('summary', $row['created_date'], '', 'm/d/Y h:i:s A', '');
                $salesProposalStatus = $salesProposalModel->getSalesProposalStatus($row['sales_proposal_status']);
                                  ...
                                  <div class="row g-1">
                                        <div class="col-8">
                                            <a href="sales-proposal-for-ci.php?customer='. $securityModel->encryptData($customerID) .'&id='. $salesProposalIDEncrypted .'" target="_blank" class="text-dark text-decoration-none">
                                                ...
                                                <p class="text-muted mb-0"><small>Product Type: '. $productType .'</small></p>
                                                <p class="text-muted mb-0"><small>Stock Number: '. $stockNumber .'</small></p>
                                                <p class="text-muted mb-0"><small>CI Verification Date: '. $forCIDate .'</small></p>
                                                <p class="text-muted mb-0"><small>CI Verification: '. $ci_recommendation .'</small></p>
                                            </a>
                                      </div>
                                      <div class="col-4 text-end">
                                          '. $salesProposalStatus .'
            ...
            if(isset($_POST['filter_release_date_start_date']) && isset($_POST['filter_release_date_end_date'])){
                ...
                foreach ($options as $row) {
                    ...
                    $response[] = [
                        ...
                        'CUSTOMER' => '<div class="col text-wrap">
                                            <h6 class="mb-0">'. $customerName .'</h6>
                                            <p class="f-12 mb-0">'. $corporateName .'</p>
                                        </div>',
                        'DR_NUMBER' => $drNumber,
                        'PRODUCT_TYPE' => $productType,
                        'PRODUCT' => $stockNumber,
                        'RELEASED_DATE' => '<div class="text-wrap">'. $forDRDate .'</div>',
                        'STATUS' => $salesProposalStatus,
                        'ACTION' => '<div class="d-flex gap-2">
        ...
        case 'sales proposal approval condition table':
            if(isset($_POST['sales_proposal_id']) && !empty($_POST['sales_proposal_id'])){
                $salesProposalID = htmlspecialchars($_POST['sales_proposal_id'], ENT_QUOTES, 'UTF-8');

                $updateApprovalConditionStatus = $userModel->checkSystemActionAccessRights($user_id, 228);
                $deleteApprovalCondition = $userModel->checkSystemActionAccessRights($user_id, 229);
                $updateApprovalCondition = $userModel->checkSystemActionAccessRights($user_id, 230);

                $sql = $databaseModel->getConnection()->prepare('SELECT * FROM sales_proposal_condition WHERE sales_proposal_id = :salesProposalID');
                $sql->bindValue(':salesProposalID', $salesProposalID, PDO::PARAM_INT);
                $sql->execute();
                $options = $sql->fetchAll(PDO::FETCH_ASSOC);
                $sql->closeCursor();
                ...
                foreach ($options as $row) {
                    $sales_proposal_condition_id = $row['sales_proposal_condition_id'];
                    $approval_condition = $row['approval_condition'];
                    $condition_type = $row['condition_type'];
                    $condition_status = $row['condition_status'];

                    $action = '';

                    if($updateApprovalCondition['total'] > 0 && $condition_status == 'Pending'){
                        $action .= '<button type="button" class="btn btn-icon btn-success update-sales-proposal-condition" data-bs-toggle="offcanvas" data-bs-target="#sales-proposal-condition-offcanvas" aria-controls="sales-proposal-condition-offcanvas" data-sales-proposal-condition-id="'. $sales_proposal_condition_id .'" title="Update Approval Condition">
                                        <i class="ti ti-edit"></i>
                                    </button>';
                    }

                    if($updateApprovalConditionStatus['total'] > 0 && $condition_status == 'Pending'){
                        $action .= ' <button type="button" class="btn btn-icon btn-success complete-sales-proposal-condition" data-sales-proposal-condition-id="'. $sales_proposal_condition_id .'" title="Complete Approval Condition">
                                        <i class="ti ti-check"></i>
                                    </button>
                                    <button type="button" class="btn btn-icon btn-warning waive-sales-proposal-condition" data-sales-proposal-condition-id="'. $sales_proposal_condition_id .'" title="Waive Approval Condition">
                                        <i class="ti ti-x"></i>
                                    </button>';
                    }

                    if($deleteApprovalCondition['total'] > 0 && $condition_status == 'Pending'){
                        $action .= '
                        <button type="button" class="btn btn-icon btn-danger delete-sales-proposal-condition" data-sales-proposal-condition-id="'. $sales_proposal_condition_id .'" title="Delete Approval Condition">
                            <i class="ti ti-trash"></i>
                        </button>';
                    }
