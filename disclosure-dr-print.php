<?php
    ...
    $pdf->MultiCell(0, 0, '<b>DISCLOSURE STATEMENT OF LOAN/ CREDIT TRANSACTION</b>', 0, 'C', 0, 1, '', '', true, 0, true, true, 0);
    $pdf->Ln(1);
    ...
    $pdf->Cell(120, 8, strtoupper($customerAddress), 0, 0, 'L');
    $pdf->Ln(10);
    ...
    $pdf->Cell(32, 8, 'P   ' . number_format($transactionFee, 2) , 0, 0, 'L');
    $pdf->Ln(8);
    $pdf->Cell(10, 8, '       '  , 0, 0, 'L');
    ...
    $pdf->Ln(8);
    $pdf->Cell(10, 8, '       '  , 0, 0, 'L');
    $pdf->Cell(80, 8, "b. Attorney's fee - 30% of total amount due; liquidated damages- further sum of 20% in addition to costs and other litigation expenses." , 0, 0, 'L', '', 1);
    $pdf->Ln(8);
    $pdf->Cell(10, 8, '       '  , 0, 0, 'L');
    ...
    function generateOtherCharges($databaseModel, $systemModel, $salesProposalID){
        ...
        <thead>
            <tr>
                <th width="25%"><b>ACCOUNT NUMBER</b></th>
        ...
        if($resultCount > 0){
            foreach ($options as $row) {
                ...
                $response .= '<tr>
                                <td>'. strtoupper($accountNumber) .'</td>
